...
module.exports = (vars = {}) => {
	return {
		...
		columnOffset(keyword, share, columns = vars.grid.columns, margin = (toPercentage(toNumber(vars.grid.margin) / 2))) {
			...
			if (keyword === 'spaced') {
				// return decl('margin-left', `(${left}) + ${toPercentage(toNumber(margin) * 2)}`);
				return decl('margin-left', toPercentage(left + toNumber(margin) * 2));
			}
